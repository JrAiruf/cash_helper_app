  ...
  Future<void>? resetUserPassword(String? email, String? operatorCode, String? enterpriseId, String? collection, String? newPassword) async {
    try {
      ...
      if (_dataVerifier.validateInputData(inputs: [email, operatorCode])) {
        ...
        final operatorsCollection = _database.collection(databaseOperator["operatorOcupation"]);
        await operatorsCollection.doc(databaseOperator["operatorId"]).update({"operatorPassword": newPassword!});
      } else {
        return;
      }
...
void main() {
  ...
  group(
    ...
    () {
      test("Create an operator in database", () async {
        ...
        final result = await firebaseMock.collection("enterprise").doc(createdEnterprise["enterpriseId"]).collection(createdOperator?["businessPosition"]).get();
      ...
      test("Fail to create the operator document in firebase", () async {
        expect(() async => database.register(LoginTestObjects.newOperator, "", "operator"), throwsA(isA<RegistrationError>()));
      });
    },
  );
  group("Register function should", () {
    test("Create a manager in database", () async {
      ...
      final enterprisesList = await firebaseMock.collection("enterprise").get();
      final createdEnterprise = enterprisesList.docs.first.data();
      final createdManager = await database.register(LoginTestObjects.newManager, createdEnterprise["enterpriseId"], LoginTestObjects.newManager["businessPosition"]);
      final result = await firebaseMock.collection("enterprise").doc(createdEnterprise["enterpriseId"]).collection(createdManager?["businessPosition"]).get();
      expect(result.docs.isEmpty, equals(false));
    ...
    test("Fail to create the manager document in firebase", () async {
      expect(() async => database.register(LoginTestObjects.modifiedUser, "", "manager"), throwsA(isA<RegistrationError>()));
    });
  });
    ...
    () {
      test(
        ...
        () async {
          ...
          final loginOperator = await database.login(createdOperator?["operatorEmail"], "ahflajdfjlakwee", createdEnterprise?["enterpriseId"], createdOperator?["businessPosition"]);
          expect(loginOperator != null, equals(true));
          expect(loginOperator?["operatorId"] == createdOperator?["operatorId"], equals(true));
      ...
      test(
        ...
        () async {
          ...
          final createdEnterprise = enterprisesList.docs.first.data();
          final createdOperator = await database.register(LoginTestObjects.newOperator, createdEnterprise["enterpriseId"], LoginTestObjects.newOperator["businessPosition"]);
          expect(() async => database.login(createdOperator?["operatorEmail"], "", "", ""), throwsA(isA<UserNotFound>()));
        },
      );
      test(
        "Fail to sign in with non-existing user",
        () async {
          expect(() async => database.login("nonUserEmail", "", "", ""), throwsA(isA<UserNotFound>()));
        },
      );
      test(
        ...
        () async {
          ...
          final createdEnterprise = enterprisesList.docs.first.data();
          final createdManager = await database.register(LoginTestObjects.newManager, createdEnterprise["enterpriseId"], LoginTestObjects.newManager["businessPosition"]);
          expect(() async => database.login(createdManager?["managerEmail"], createdManager?["managerPassword"], createdEnterprise["enterpriseId"], "otherposition"), throwsA(isA<UserNotFound>()));
        },
      );
      test(
        ...
        () async {
          ...
          final createdEnterprise = enterprisesList.docs.first.data();
          final createdManager = await database.register(LoginTestObjects.newManager, createdEnterprise["enterpriseId"], LoginTestObjects.newManager["businessPosition"]);
          expect(() async => database.login(createdManager?["managerEmail"], "wrongPassword", createdEnterprise["enterpriseId"], createdManager?["businessPosition"]), throwsA(isA<UserNotFound>()));
        },
      );
    },
  );
  test(
    ...
    () async {
      ...
      final enterprisesList = await firebaseMock.collection("enterprise").get();
      final createdEnterprise = enterprisesList.docs.first.data();
      final createdManager = await database.register(LoginTestObjects.loginManager, createdEnterprise["enterpriseId"], LoginTestObjects.newManager["businessPosition"]);
      final loginOperator = await database.login(createdManager?["managerEmail"], "34hahlfakdf43h", createdEnterprise["enterpriseId"], createdManager?["businessPosition"]);
      expect(loginOperator != null, equals(true));
    ...
    () {
      test(
        ...
        () async {
          ...
          final createdEnterprise = enterprisesList.docs.first.data();
          final createdOperator = await database.register(LoginTestObjects.newOperator, createdEnterprise["enterpriseId"], LoginTestObjects.newOperator["businessPosition"]);
          final result = await database.getUserById(createdEnterprise["enterpriseId"], createdOperator?["operatorId"], createdOperator?["businessPosition"]);
      ...
      test(
        ...
        () async {
          ...
          final createdEnterprise = enterprisesList.docs.first.data();
          final createdManager = await database.register(LoginTestObjects.newManager, createdEnterprise["enterpriseId"], LoginTestObjects.newManager["businessPosition"]);
          final result = await database.getUserById(createdEnterprise["enterpriseId"], createdManager?["managerId"], createdManager?["businessPosition"]);
          expect(result, isA<Map<String, dynamic>>());
...
/* 
  ...
  test(
    ...
    () async {
      final createdEnterprise = await enterpriseDatabase.createEnterpriseAccount(EnterpriseTestObjects.enterpriseMap);
      final createdOperator = await database.register(LoginTestObjects.signOutOperator, createdEnterprise?["enterpriseId"], LoginTestObjects.newOperator["businessPosition"]);

      await database.login(createdOperator?["operatorEmail"], "ahflajdfd423636akwee", createdEnterprise?["enterpriseId"], createdOperator?["businessPosition"]);
